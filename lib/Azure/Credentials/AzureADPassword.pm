package Azure::Credentials::AzureADPassword;
  use Moose;
  use JSON::MaybeXS;
  use Path::Class::File;
  ...
  has username => (
    ...
    required => 1,
    default => sub {
      $ENV{ AZURE_USERNAME } // die "Expected setting username in AzureADPassword constructor or env AZURE_USERNAME";
    },
  );
  ...
  has password => (
    ...
    required => 1,
    default => sub {
      $ENV{ AZURE_PASSWORD } // die "Expected setting password in AzureADPassword constructor or env AZURE_PASSWORD";
    },
  );
