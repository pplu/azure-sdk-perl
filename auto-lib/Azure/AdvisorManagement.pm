package Azure::AdvisorManagement;
  ...
  sub GetGenerateRecommendationsStatusRecommendations {
    my $self = shift;
    if (defined $self->subscription_id) {
      push @_, 'subscriptionId' => $self->subscription_id;
    }
    my $call_object = $self->new_with_coercions('Azure::AdvisorManagement::GetGenerateRecommendationsStatusRecommendations', { @_ });
    return $self->caller->do_call($self, $call_object);
  }
  ...
  sub operations { qw/CreateSuppressions DeleteSuppressions GenerateRecommendations GetGenerateRecommendationsStatusRecommendations GetRecommendations GetSuppressions ListOperations ListRecommendations ListSuppressions / }
